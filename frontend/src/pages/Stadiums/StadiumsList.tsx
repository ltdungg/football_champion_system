...
const StadiumsList: React.FC = () => {
  ...
  // States for a modal window
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [editingStadium, setEditingStadium] = useState<Stadium | null>(null);

  // <-- CHANGE: Create "deferred" search value
  const debouncedSearchTerm = useDebounce(searchTerm, 500);

  const fetchStadiums = async () => {
    // We do not install isLoading V true here to avoid the entire list from blinking when searching
    setError(null);
    try {
      // <-- CHANGE: Passing the deferred search request to API
      ...
      // Install isLoading V false only on first boot
  ...
  useEffect(() => {
    fetchStadiums();
  }, [debouncedSearchTerm]); // <-- CHANGE: useEffect depends on debouncedSearchTerm
  ...
  // <-- CHANGE: Client .filter() deleted All that's left is sorting.
