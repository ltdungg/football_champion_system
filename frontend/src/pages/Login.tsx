...
import { useAuth } from '../context/AuthContext'; // <-- path '../' correct
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 flex items-center justify-center p-4">
      <div className="relative bg-white/10 backdrop-blur-lg rounded-3xl p-8 w-full max-w-md border border-white/20 shadow-2xl">
        <div className="text-center mb-8">
          ...
          <h1 className="text-3xl font-bold text-white mb-2">Login to account</h1>
          <p className="text-gray-300">Login to the management system</p>
        </div>
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <Button type="submit" variant="primary" size="lg" fullWidth loading={isSubmitting || isAuthLoading} icon={LogIn} className="bg-gradient-to-r from-cyan-500 to-blue-600 hover:from-cyan-600 hover:to-blue-700">
            {isSubmitting || isAuthLoading ? 'Entrance...' : 'Login'}
          </Button>
        </form>
        <div className="mt-6 text-center">
            <p className="text-gray-400 text-sm">No account yet?{' '}<button onClick={() => navigate('/register')} className="text-cyan-400 hover:text-cyan-300 font-medium">Create it here</button></p>
        </div>
      </div>
    </div>
  );
