...
const MatchesList: React.FC = () => {
  ...
  const fetchMatches = async () => {
    // We don't reset isLoading, so that it doesn't happen "flashing" when filtering
    setError(null);
    try {
      // The backend does not support match filtering, so we filter on the client
      const data = await getMatches(0, 1000); // Loading all matches
      setMatches(data);
    } catch (err) {
