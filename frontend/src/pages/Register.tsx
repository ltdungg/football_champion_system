...
import { Eye, EyeOff, UserPlus, Mail, User, Lock } from 'lucide-react'; // CHANGED: 'Trophy' deleted
...
const Register: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(err.response?.data?.detail || 'Registration error. Perhaps such a user already exists.');
    } finally {
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 flex items-center justify-center p-4">
      <div className="relative bg-white/10 backdrop-blur-lg rounded-3xl p-8 w-full max-w-md border border-white/20 shadow-2xl">
        <div className="text-center mb-8">
          ...
          <p className="text-gray-300">Join the management system</p>
        </div>
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <Button type="submit" variant="primary" size="lg" fullWidth loading={isSubmitting || isAuthLoading} icon={UserPlus} className="bg-gradient-to-r from-green-500 to-emerald-600 hover:from-green-600 hover:to-emerald-700">
            {isSubmitting || isAuthLoading ? 'Creation...' : 'Create an account'}
          </Button>
        </form>
        <div className="mt-6 text-center"><p className="text-gray-400 text-sm">Already have an account?{' '}<button onClick={() => navigate('/login')} className="text-cyan-400 hover:text-cyan-300 font-medium">Login here</button></p></div>
      </div>
    </div>
  );
};
